    ...
    try:
        ...
        if os.path.exists(history_file):
            ...
            with open(history_file, 'r') as f:
                history = json.load(f)
            # Return reversed to show newest first if not already
...
def log_client_event():
    ...
    try:
        ...
        # Log with specific tag to make it easy to grep
        logger.info(f"🖱️ CLIENT_EVENT [{event_type}]: {details} @ {timestamp}")
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print(f"Opening GUI at: http://localhost:5003")
